...
interface SimpleVoiceOnboardingModalProps {
  onComplete?: (data: { 
    goals?: Array<{ 
      original: string; 
      text?: string; 
...
export function SimpleVoiceOnboardingModal({ 
  ...
  goalContext,
  onComplete
}: SimpleVoiceOnboardingModalProps) {
  ...
  useEffect(() => {
    return () => {
      ...
      if (conversation.status === 'connected' || conversation.status === 'connecting') {
        console.log('SimpleVoiceOnboardingModal: Cleaning up conversation on unmount');
    ...
    <div className="w-full">
      {agentDetails && (
        <div className="mb-4 text-center">
          ...
          <h3 className="text-xl font-semibold">{agentDetails.Name}</h3>
          <p className="text-sm text-gray-600 dark:text-gray-400">
            {agentDetails.Speciality}
          </p>
          {agentDetails.Personality && (
            <div className="inline-block px-2 py-1 mt-2 bg-secondary text-secondary-foreground rounded-md text-xs">
              {agentDetails.Personality}
            </div>
          )}
        </div>
      ...
      {/* Messages */}
      {messages.length > 0 && (
        <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-3 max-h-32 overflow-y-auto mb-4">
